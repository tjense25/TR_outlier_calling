...
calc_VNTR_outliers <- function(samples, haplotypes, repeat_length, VNTR_ID, k=25) {
    TR_catalog <- ref_TR_catalog[ref_TR_catalog$VariantId %in% VNTR_ID, ]
    catalog_lengths <- as.numeric(str_split_1(TR_catalog$RepeatNumbers,','))
    catalog_counts <- as.numeric(str_split_1(TR_catalog$AlleleCounts, ','))
    ...
    return(data.frame(samples,haplotypes,repeat_length,rank=rank(-combined_lengths)[1:repeat_length],mnd=mean_neighbor_distance, outlier, mnd_sigmas=outlier_sd, repeat_length_mean=mean(combined_lengths), repeat_length_sd=sd(combined_lengths)))
}
...
chunked_ids <- chunked_ids[1:16]
outliers <- Reduce(rbind,pbmclapply(chunked_ids, function(x) vntrs %>% filter(VNTR_ID %in% x) %>% group_by(VNTR_ID) %>% reframe(calc_VNTR_outliers(sample, haplotype, size, VNTR_ID, k=k))))
outliers$mnd
